...
export const EvidenceViewer = ({
  ...
  const [activeTab, setActiveTab] = useState('side-by-side'); // 'side-by-side' | 'uploaded' | 'matched'

  return (
    <div className={cn('flex flex-col bg-slate-900 border border-slate-800 rounded-xl overflow-hidden', className)}>
      {/* Header Banner */}
      <div className="flex flex-wrap items-center justify-between p-4 bg-slate-950 border-b border-slate-800 gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Deep Convolutional Feature Alignment & EXIF Forensic Engine</p>
          </div>
        </div>
      ...
      <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-6 bg-slate-900/60">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="relative rounded-lg overflow-hidden border-2 border-slate-700 bg-slate-950 aspect-[4/3] group">
            <img
              src={uploadedImage}
              alt="Uploaded Project Progress"
              className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
            />
            <div className="absolute top-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded text-xs font-mono text-blue-400 border border-slate-700">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-rose-300 flex items-center gap-1.5">
              ...
              2. Matched Historical / Prior Project Photo
            </span>
          ...
          <div className="relative rounded-lg overflow-hidden border-2 border-rose-500/50 bg-slate-950 aspect-[4/3] group shadow-glow-red">
            <img
              src={matchedImage}
              alt="Matched Prior Project Photo"
              className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
            />
            <div className="absolute top-3 left-3 bg-rose-950/90 backdrop-blur-md px-2.5 py-1 rounded text-xs font-mono text-rose-300 border border-rose-800">
              Archived: 12 Nov 2024 (Jaunpur District)
            </div>
            <div className="absolute bottom-3 right-3 bg-rose-600 text-white px-3 py-1 rounded-md text-xs font-bold font-mono shadow-lg">
              96% Exact Structure Match
            </div>
          </div>

          {/* Matched Meta */}
          <div className="bg-slate-950/60 border border-rose-900/40 rounded-lg p-3 space-y-1.5 text-xs text-slate-300">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 flex items-center gap-1.5">
                <FileSearch className="w-3.5 h-3.5 text-rose-400" /> Source Work:
              </span>
              <span className="font-medium text-slate-200">{matchedMeta.sourceProjectName || 'Gram Panchayat Road, Jaunpur'}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-400 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-rose-400" /> Match Method:
              </span>
              <span className="font-mono text-rose-300">{matchedMeta.matchType || 'pHash & ResNet-50 Feature Distance < 0.04'}</span>
            </div>
            <div className="flex items-center justify-between">
