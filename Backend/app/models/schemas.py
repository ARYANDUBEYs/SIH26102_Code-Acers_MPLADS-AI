...
class ProjectRecord(BaseModel):
    ...
    sanctioned_amount: float = Field(..., description="Amount sanctioned in INR", example=4800000.0)
    funds_released: float = Field(..., description="Amount released to implementing agency in INR", example=3850000.0)
    funds_utilized: float = Field(..., description="Expenditure incurred so far in INR", example=3710000.0)
    ...
    days_elapsed: int = Field(..., example=215)
    allocated_duration_days: int = Field(..., example=225)
    contractor_id: str = Field(..., example="VEN-8821")
    contractor_name: str = Field(..., example="ABC Infra Constructions Pvt Ltd")
    latitude: float = Field(..., example=21.3851)
    longitude: float = Field(..., example=74.9023)
