...
class ProjectStore:
    ...
    async def update_project(self, project_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if self._use_mongo:
            return await self._collection.find_one_and_update({"project_id": project_id}, {"$set": updates}, projection={"_id": 0}, return_document=True)
        for project in self._memory_store:
